...
<step n="5.5" goal="Criar/atualizar arquivo .env com variáveis de volume">
<action>Verificar se arquivo .env existe no projeto</action>

<check if=".env exists">
<action>Ler conteúdo atual do .env</action>
<action>Verificar quais variáveis de volume já existem</action>
</check>
...
<action>Adicionar ou atualizar variáveis de volume no .env</action>
...
<template-output>env_volume_vars</template-output>
</step>
